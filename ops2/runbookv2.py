...
<pre>services:
  minecraft:
    image: 998487032255.dkr.ecr.us-east-1.amazonaws.com/cs312-minecraft:mc-1.20.4-build1
    ports:
      - "25565:25565"
    environment:
      EULA: "TRUE"
    ...
    <h2>4. Build & Publish Workflow</h2>
...
docker push 998487032255.dkr.ecr.us-east-1.amazonaws.com/cs312-minecraft:mc-1.20.5-build1</pre>
    ...
    <p>Backups are stored in a private S3 bucket (<code>cs312-mc-backups-998487032255</code>). A bucket lifecycle rule expires artifacts older than 7 days. This 7-day retention period was chosen because it minimizes AWS storage costs while providing ample coverage for game-state rollback; in the context of a game server, data older than a week usually represents too much lost player progression to be a viable recovery point.</p>
...
docker compose up -d</pre>
    ...
    <h2>7. Initial Deployment Runbook</h2>
    <p>Follow these steps to deploy the server from scratch:</p>
    <ol>
        <li><strong>Provision EC2:</strong> Launch a t2.medium instance with Ubuntu.</li>
        <li><strong>Security Group:</strong> Open TCP port 25565 for Minecraft traffic.</li>
        <li><strong>Install Docker:</strong> Install Docker and Docker Compose on the host.</li>
        <li><strong>Directory Setup:</strong> Run <code>mkdir -p ~/minecraft-server/data</code> and <code>cd ~/minecraft-server</code>.</li>
        <li><strong>Compose File:</strong> Create the <code>docker-compose.yml</code> file with the service configuration.</li>
        <li><strong>Authenticate:</strong> Log in to the AWS ECR registry.</li>
        <li><strong>Start Service:</strong> Run <code>docker compose up -d</code>.</li>
    </ol>

    <h2>8. Failure Scenarios</h2>
    <ul>
        <li><strong>Container Crash Loop:</strong> Container repeatedly fails to start. <strong>Response:</strong> Check logs with <code>docker compose logs</code>. Verify EULA acceptance and allocated memory.</li>
        <li><strong>World Data Corruption:</strong> Server crashes loading chunks. <strong>Response:</strong> Follow the <strong>Restore Procedure</strong> using the latest S3 backup.</li>
        <li><strong>Connection Refused:</strong> Players cannot connect. <strong>Response:</strong> Verify the AWS Security Group allows TCP 25565 and the container port binding is correct.</li>
    </ul>

    <h2>9. Basic Monitoring</h2>
    <ul>
        <li><code>docker compose ps</code>: Verifies container uptime and state. <strong>Look for:</strong> State should be "Up".</li>
        <li><code>docker compose logs --tail=50 -f</code>: Streams recent server logs. <strong>Look for:</strong> Warnings, errors, or crash stack traces.</li>
        <li><code>docker stats</code>: Shows real-time resource usage. <strong>Look for:</strong> High memory consumption that might lead to OOM kills.</li>
    </ul>

    <h2>10. Video Demonstration</h2>
